...
@section('content')
    @php
        $statusClass = function ($status) {
            ...
            if (str_contains($s, 'diterima') || str_contains($s, 'lolos') || str_contains($s, 'accepted')) {
                return 'border-green-200 bg-green-50 text-green-800';
            }

            if (str_contains($s, 'ditolak') || str_contains($s, 'gagal') || str_contains($s, 'rejected')) {
            ...
            if (str_contains($s, 'review') || str_contains($s, 'diproses') || str_contains($s, 'proses')) {
    ...
    <div class="min-h-screen bg-gray-50">
        <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
                <div class="min-w-0">
                    <h1 class="text-2xl font-semibold tracking-tight text-gray-900">Lamaran</h1>
                    <p class="mt-1 text-sm text-gray-600 truncate">
                        {{ $job->judul }}
                        @if(!empty($job->lokasi)) • {{ $job->lokasi }} @endif
                        @if(!empty($job->tipe)) • {{ $job->tipe }} @endif
                    </p>
                </div>
            ...
            <div class="mt-8 bg-white border border-gray-200 rounded-3xl shadow-sm overflow-hidden">
                <div class="px-6 py-5 border-b border-gray-200">
                    ...
                    <p class="mt-1 text-xs text-gray-600">Cari kandidat dan saring berdasarkan status/riwayat pendidikan.</p>
                </div>

                <form method="GET" action="{{ route('hrd.jobs.applications.index', $job->id) }}" class="p-6 grid grid-cols-1 sm:grid-cols-12 gap-4">
                    <div class="sm:col-span-4">
                        ...
                        <input type="text" name="q" value="{{ request('q') }}"
                               ...
                               placeholder="Nama / email / jurusan / portofolio">
                    </div>
            ...
            <div class="mt-6 bg-white border border-gray-200 rounded-3xl shadow-sm overflow-hidden">
                <div class="px-6 py-5 border-b border-gray-200 flex items-center justify-between">
                    ...
                    <p class="text-xs text-gray-500">Total: {{ $applications->total() }}</p>
                </div>
